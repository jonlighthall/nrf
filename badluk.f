...
c     adjustment.
 ...
 1                call print_stats('LINE1',IYYY,IM,jday,JD,N,ic,icon)
     ...
     &                       )'found first bad day!',im,'/',13,'/',iyyy
                        ...
                        if(check)write(*,'(a,i0.2,a,i0.2,a)',advance
     &                       ='no')' found a bad day at ',HOUR,':',MIN
     &                       ,'. checking...'
                        newbad = .true.
                        do i=1,badtotal ! check
                           if((iyyy.eq.bads(i,1)).and.(im.eq.bads(i,2)))
     &                          then
                              whichbad = i
     ...
     &Increase array size >',badtotal
                     ...
                     write(sftimes(whichbad,timezone),'(i0.2,a
     &,i0.2)')HOUR,':',MIN
                     if((FFRAC.gt.1).and.(FFRAC.lt.1.5)) then
                        if(check) then
                           write(*,'(a,i4,a,i2,a,a5,a,a5)'
     &                          )'  found margin: yr=',iyyy,' m=',im
     &                          ,' t0=',sftimes(whichbad,timezone)
     &                          ,'; t=',sftimes(whichbad,timezone-1)
     ...
     &                       ' hrs after midnight (',zn(timezone),')'
                        if((.not.rollback).and.dofrac) then
                           write (*
     &                          ,'(1x,a,f4.1,a,a,a,i2,a,i2,a,i0.2
     &,f0.1)')'Full moon ',FFRAC,' hrs after midnight (',zn(timezone),')
     &                          at',HOUR,':',MIN,'
     &                          :',int(SEC),SEC-real(int(SEC))
                        endif
                     endif
                     badcount = badcount + 1
 ...
 10   continue
     ...
     &        ,' bad days'
         if(.not.dofrac) then   ! print hours only
            ...
            write (fmt,'(a,i2,a)')'(1x,i2,1x,i2,a,i2,a,i4,',ntz
     &           ,'(1x,a3))'
